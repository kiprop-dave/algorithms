...
Your EventEmitter class should have the following two methods:
    ...
    each should be called in the order in which they were subscribed. An array of results should be returned. You can assume
    no callbacks passed to subscribe are referentially identical.
...
class EventEmitter {
  eventMap:Map<string,Set<Callback>> = new Map();
  ...
  emit(eventName: string, args: any[] = []): any {
    const subs = this.eventMap.get(eventName);
    if(!subs) return [];
    let res:any[] = [];
    subs.forEach((cb) => {
      res = [...res,cb(...args)]
